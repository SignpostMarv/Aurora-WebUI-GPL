...
<?php	} ?>
			...
			<form method=post>
				...
				<button type=submit><?php echo esc_html(__('Save Notes')); ?></button>
			</form>
			<form method=post>
				<?php if(isset($FormProblems['nonce']) === true){ ?><p class=problem><?php echo esc_html($FormProblems['nonce']); ?></p><?php } ?><input type=hidden name=nonce value="<?php echo esc_attr(Globals::i()->Nonces->get(300)); ?>">
				<ul>
					<li><button type=submit name=delete><?php echo esc_html(__('Delete Report')); ?></button></li>
					<li><button type=submit name=<?php echo esc_attr($AbuseReport->Active() ? 'deactivate' : 'activate'); ?>><?php echo esc_html($AbuseReport->Active() ? 'Deactive Abuse Report' : 'Activate Abuse Report'); ?></button></li>
				</ul>
			</form>
		</details>
	</section>
